...
key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    else if (key == GLFW_KEY_LEFT && action == GLFW_PRESS) { // Make object thinner
        if (scale.x >= 0.1) {
            scale.x *= 0.9;
        }
    }
    else if (key == GLFW_KEY_RIGHT && action == GLFW_PRESS) { // Make object fatter
        scale.x *= 1.1;
    }
    else if (key == GLFW_KEY_UP && action == GLFW_PRESS) { // Make object taller
        scale.y *= 1.1;
    }
    else if (key == GLFW_KEY_DOWN && action == GLFW_PRESS) { // Make object shorter
        if (scale.y >= 0.1) {
            scale.y *= 0.9;
        }
    }
...
mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        // Check if any modifier keys were active at the time of the button press, e.g. GLFW_MOD_ALT, etc.
        if (mods == GLFW_MOD_CONTROL) { // Translation
            state = "Translate";
        }
        else { // Rotation
            state = "Rotate";
...
cursor_pos_callback(GLFWwindow* window, double xpos, double ypos) {
    // determine the direction of the mouse or cursor motion and update mouse position
    FloatType2D mouseDir;
    mouseDir.x = xpos - mouse_x;
    mouseDir.y = ypos - mouse_y;
    ...
    if (state == "Rotate") {
        ...
        centroid.x += mouseDir.x / window_width;
        centroid.y -= mouseDir.y / window_height;
    }
}
...
calculateM() {
    ...
    M[0] = cos(thetaRad);  M[4] = -sin(thetaRad);
    M[1] = sin(thetaRad);  M[5] = cos(thetaRad);

    // Scaling
    M[0] *= scale.x;  M[4] *= scale.y;
    M[1] *= scale.x;  M[5] *= scale.y;
    M[2] *= scale.x;  M[6] *= scale.y;
    M[3] *= scale.x;  M[7] *= scale.y;
}
...
int main(int argc, char** argv) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // define/re-define the modelview matrix
        // In this template, we define M to be the identity matrix
        // You will need define M according to the user's actions
        calculateM();
